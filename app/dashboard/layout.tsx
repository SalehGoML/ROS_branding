...
const navItems = [
  ...
  { href: '/dashboard/reports',  label: 'گزارش‌ها',           icon: 'M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8zM14 2v6h6M16 13H8M16 17H8M10 9H8' },
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--c-bg)', paddingTop: 0 }}>
      ...
      <aside style={{
        ...
        {/* Logo */}
        <div style={{
          padding: collapsed ? '1.25rem .75rem' : '1.25rem 1.25rem',
          ...
          {!collapsed && (
            <Link href="/" style={{
              ...
              <span style={{ fontWeight: 700, fontSize: '.95rem', color: 'var(--c-text)' }}>ROS</span>
            </Link>
          )}
          ...
          {navItems.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href} style={{
                display: 'flex', alignItems: 'center',
                gap: '.75rem',
                padding: collapsed ? '.7rem' : '.7rem .85rem',
        ...
        <div style={{ padding: '.75rem .5rem', borderTop: '1px solid var(--c-border)' }}>
          <button style={{
            ...
            width: '100%', padding: collapsed ? '.7rem' : '.7rem .85rem',
            justifyContent: collapsed ? 'center' : 'flex-start',
            borderRadius: 'var(--r-sm)',
            background: 'transparent', border: 'none',
            color: 'var(--c-text-muted)', fontSize: '.85rem',
            cursor: 'pointer',
          }}>
            <svg width="16" height="16" viewBox="0 0 24 24"
