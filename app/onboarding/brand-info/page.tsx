...
const industries = [
  'صنایع غذایی', 'سلامت و بهداشت', 'آموزش', 'تکنولوژی',
  'خدمات مالی', 'خرده‌فروشی', 'ساختمان و مسکن',
  'حمل‌ونقل', 'رسانه و سرگرمی', 'سایر',
]

export default function BrandInfoPage() {
  const router = useRouter()
  const [form, setForm] = useState({
    brandName: '', website: '', industry: '', hasStrategy: '',
  })
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div>
              <label style={{ fontSize: '.78rem', fontWeight: 500, color: 'var(--c-text-muted)', display: 'block', marginBottom: '.4rem' }}>
                ...
                <span style={{ color: 'var(--c-text-light)', fontWeight: 400, marginRight: '.3rem' }}>(اختیاری)</span>
              </label>
              <input
                value={form.website}
            ...
            {/* Industry */}
            <div>
              <label style={{ fontSize: '.78rem', fontWeight: 500, color: 'var(--c-text-muted)', display: 'block', marginBottom: '.65rem' }}>
                حوزه فعالیت برند <span style={{ color: 'var(--c-primary)' }}>*</span>
              </label>
            ...
            <div>
              <label style={{ fontSize: '.78rem', fontWeight: 500, color: 'var(--c-text-muted)', display: 'block', marginBottom: '.65rem' }}>
                آیا برند شما دارای سند استراتژیک است؟ <span style={{ color: 'var(--c-primary)' }}>*</span>
              </label>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '.75rem' }}>
                {[
                  ...
                  <div
                    ...
                    style={{
                      padding: '1rem',
                      background: form.hasStrategy === opt.value ? 'var(--c-primary-bg)' : 'var(--c-surface-2)',
                  ...
                  >
                    <div style={{
                      ...
                      color: form.hasStrategy === opt.value ? 'var(--c-primary)' : 'var(--c-text)',
                      marginBottom: '.2rem',
                    }}>
                      {opt.label}
                    </div>
                    <div style={{ fontSize: '.75rem', color: 'var(--c-text-muted)' }}>
                      {opt.desc}
                    </div>
                  </div>
                ))}
          ...
          <button
            ...
            style={{
              ...
              padding: '.9rem',
              background: isValid && !loading ? 'var(--c-primary)' : 'var(--c-border)',
